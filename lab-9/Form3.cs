...
namespace lab_9
{
    public partial class Form3 : Form
    {
        ...
        {
            InitializeComponent();
            sqlConnection = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=E:\ООП\lab-9\lab-9\Database2.mdf;Integrated Security=True");
            sqlConnection.Open();
        ...
        private void Form3_Load(object sender, EventArgs e)
        {
            ...
            SqlCommand sqlcommand = new SqlCommand($"SELECT Имя,Фамилия,Отчество,Телефон FROM Test WHERE Id={DataBank.Id}", sqlConnection);
            ...
            while (sqlDataReader.Read())
            {
                ...
                textBox1.Text = ($"{sqlDataReader["Фамилия"]}");
                    ...
                    if (resultSName == false)
                    {
                        ...
                        if (resultName == false)
                        {
                            ...
                            if (resultMName == false)
                            {
                                ...
                                SqlCommand sqlcommandupdate = new SqlCommand($"UPDATE Test SET Имя=N'{textBox2.Text}', Фамилия=N'{textBox1.Text}', Отчество=N'{textBox3.Text}', Телефон={textBox4.Text} WHERE Id={DataBank.Id}"
                                        , sqlConnection);
